...
from PySide6.QtWidgets import QHBoxLayout, QLabel, QPlainTextEdit, QPushButton, QSlider, QVBoxLayout, QWidget
...
def create_ai_tab(dialog) -> QWidget:
    ...
    info_icon = QLabel("i")
    info_icon.setStyleSheet("font-size: 24px; font-weight: bold;")
    info_layout.addWidget(info_icon)

    info_text = QLabel(
        "Hệ thống dùng <b>Waterfall Strategy</b> với các text model tương thích.\n"
        "Tự động bỏ qua model không phù hợp và chuyển model khác nếu gặp lỗi."
    )
    ...
    dialog.creativity_slider.valueChanged.connect(lambda value: dialog.creativity_value.setText(f"{value}%"))
